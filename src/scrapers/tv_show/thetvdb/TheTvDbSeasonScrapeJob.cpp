...
{
    TvDbId id(config().showIdentifier.str());
    const auto callback = [this](QString json) {
        ...
        const auto next = m_parser.parseEpisodes(json, this, onEpisode);
        if (next.hasNextPage()) {
    ...
    if (config().shouldLoadAllSeasons()) {
        m_api.loadSeasonsPage(config().locale, id, config().seasons, config().seasonOrder, page, callback);
    } else {
        m_api.loadAllSeasonsPage(config().locale, id, config().seasonOrder, page, callback);
    }
}
...
void TheTvDbSeasonScrapeJob::storeEpisode(TvShowEpisode* episode)
{
    ...
    if (config().seasons.contains(season)) {
