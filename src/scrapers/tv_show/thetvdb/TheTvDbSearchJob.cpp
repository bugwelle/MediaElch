#include "scrapers/tv_show/thetvdb/TheTvDbSearchJob.h"
...
TheTvDbSearchJob::TheTvDbSearchJob(TheTvDbApi& api, ShowSearchJob::Config config, QObject* parent) :
    ShowSearchJob(std::move(config), parent), m_api{api}
{
}

void TheTvDbSearchJob::execute()
{
...
QVector<ShowSearchJob::Result> TheTvDbSearchJob::parseSearch(const QString& json)
{
    QJsonParseError parseError{};
    const auto parsedJson = QJsonDocument::fromJson(json.toUtf8(), &parseError).object();

    if (parseError.error != QJsonParseError::NoError) {
        qWarning() << "[TheTvDbSearchJob] Error parsing search:" << parseError.errorString();
        return {};
    }
...
ShowSearchJob::Result TheTvDbSearchJob::parseSingleSearchResult(const QJsonObject& showObject)
{
