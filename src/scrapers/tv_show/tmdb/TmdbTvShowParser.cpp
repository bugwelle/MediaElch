...
void TmdbTvShowParser::parseInfos(const QJsonDocument& json, const Locale& locale)
{
    ...
    m_show.setStatus(data["status"].toString());
    ...
    for (QJsonValueRef seasonVal : seasonArray) {
        ...
        SeasonNumber season = SeasonNumber(seasonObj["season_number"].toInt());

        Poster seasonPoster;
        seasonPoster.id = m_api.makeImageUrl(data["poster_path"].toString()).toString();
        seasonPoster.thumbUrl = showBackdrop.id;
        showBackdrop.originalUrl = showBackdrop.id;
    ...
    // -------------------------------------
    const QJsonArray networks = data["networks"].toArray();
    if (networks.size() > 0) {
        m_show.setNetwork(networks.first().toObject()["name"].toString());
    }
