    ...
    QWidget(parent), ui(new Ui::TvScraperInfoWidget), m_scraper{scraper}
{
    ...
    connect(&scraper, &mediaelch::scraper::TvScraper::initialized, this, &TvScraperInfoWidget::setupInitializedBox);
}
...
void TvScraperInfoWidget::setupInitializedBox()
{
    ui->txtInitialized->setText(m_scraper.isInitialized() ? tr("Yes") : tr("No"));
}
