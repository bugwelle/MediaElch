...
void TvShowWidgetTvShow::onStartScraperSearch()
{
    ...
    const TvDbId tvdbId = m_show->tvdbId();
    const QString search = tvdbId.isValid() ? tvdbId.withPrefix() : m_show->title();
    searchWidget->execWithSearch(search);

    const int result = searchWidget->result();
    const QString identifier = searchWidget->showIdentifier();
